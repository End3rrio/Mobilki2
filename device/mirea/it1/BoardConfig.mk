TARGET_BOARD_PLATFORM := sdm845 # определяет платформу, на которойпостроено устройство (зависит от используемого процессора)
# Architecture
...
TARGET_KERNEL_CONFIG := it_deconfig # указывает имя конфигурационного файла для сборки ядра.
...
PRODUCT_PACKAGES += \ # дополнительные пакеты, необходимые для установки A/B OTA
    update_engine \
    update_verifier
TARGET_NO_RECOVERY := false # отключение наличия раздела recovery
